...
function createUserWithPetProfileForBabelBark($request){
	...
	if (!isset($data['first_name']) || !isset($data['last_name']) || !isset($data['email']) || !isset($data['password']) || !isset($data['phone']) || empty(trim($data['first_name'])) || empty(trim($data['last_name'])) || empty(trim($data['email'])) || empty(trim($data['password'])) || empty(trim($data['phone'])) && !isset($data['pet_name']) && empty(trim($data['pet_name'])) && !isset($data['pet_type']) && empty(trim($data['pet_type'])) && !isset($data['pet_breed']) && empty(trim($data['pet_breed'])) && !isset($data['pet_primary_color']) && empty(trim($data['pet_primary_color'])) && !isset($data['pet_gender']) && empty(trim($data['pet_gender'])) && !isset($data['pet_birthday']) && empty(trim($data['pet_birthday'])) && !isset($data['pet_primary_image']) && empty(trim($data['pet_primary_image']))) {
		...
		return array("success"=>0,"message"=>"Please fill the all required fields.");
	}
	...
	if (isset($data['neutered_spayed'])) {
		$neutered = $data['neutered_spayed'];
	}else{
		$neutered = "";
	}

	if (isset($data['tag_engraving'])) {
		$engraving = $data['tag_engraving'];
	}else{
		$engraving = "";
	}

	if (isset($data['tag_size'])) {
		$tagSize = $data['tag_size'];
	}else{
		$tagSize = "";
	}
	...
	if (strlen($password) < 7) {
		return array("success"=>0,"message"=>"Password must be at least 8 characters!");
	}
	...
	if ($checkPhone['success']) {
		return array("success"=>0,"message"=>"Phone number ".$phone." already used by another user.");
	}elseif (email_exists( $emailAdd )) {
		return array("success"=>0,"message"=>"Email address ".$emailAdd." already used by another user.");
	}

	if (trim($neutered) != "yes" && trim($neutered) != "no" && !empty(trim($neutered))) {
		return array("success"=>0,"message"=>"The neutered_spayed field accepts only yes or no.");
	}

	if (trim($tagSize) != "small" && trim($tagSize) != "large" && !empty(trim($tagSize))) {
		return array("success"=>0,"message"=>"The tag_size field accepts only small or large.");
	}

	if (!empty(trim($engraving)) && strlen($engraving) > 10) {
		return array("success"=>0,"message"=>"The tag_engraving field accepts max 10 character.");
	}

	if (trim($neutered) == "yes") {
		$neutered = 1;
	}elseif (trim($neutered) == "no") {
		$neutered = 0;
	}
	...
	if ($newUserId) {
        ...
        if (!empty($neutered)) {
        	update_post_meta($postId , 'neutered_spayed', $neutered);
        }

        if (!empty($tagSize)) {
        	update_post_meta($postId , 'tag_size', $tagSize);
        }

        if (!empty($engraving)) {
        	update_post_meta($postId , 'tag_engraving', $engraving);
        }
        ...
        if (isset($data['alternate_phone_code']) && !empty(trim($data['alternate_phone_code']))) {
        	update_user_meta($newUserId,'primary_cell_country_code',$data['alternate_phone_code']);
        }

        if (isset($data['phone_code']) && !empty(trim($data['phone_code']))) {
        	update_user_meta($newUserId,'primary_phone_country_code',$data['phone_code']);
        }
		...
		return array("success"=>0,"message"=>"User not created, please try again.");
	}
...
function updateUserForBabelBark($request){	
	...
	if (isset($data['phone']) && !empty($data['phone'])) {
		...
		if ($checkPhone['success']) {
			return array("success"=>0,"message"=>"Phone number ".$data['phone'].".");
		}
	}
	...
	if (!$ownerUser) {
		return array("success"=>0,"message"=>"This email is not exist in our database.");
	}
	...
	if ($createdBy == $accountId ) {
        ...
        if (isset($data['phone_code']) && !empty(trim($data['phone_code']))) {
        	update_user_meta($newUserId,'',$data['phone_code']);
        }
        return array("success"=>1,"message"=>"User info updated successfully.");
	}else{
		return array("success"=>0,"message"=>"This user is not created by you, so you will not be able to update this user information.");
	}
}
...
function updatePetProfileForBabelBark($request){
	...
	/*if (isset($data['microchip_id']) && !empty($microchipId)) {
		return array("success"=>0,"message"=>"Microchip id is already added so this field can't be update.");
	}elseif (isset($data['smart_tag_id']) && !empty($smartTagId)) {
		return array("success"=>0,"message"=>"SmartTag id is already added so this field can't be update.");
	}*/

	if (isset($data['microchip_id']) && !empty(trim($data['microchip_id']))) {
		if(!checkMicrochipIDPetProfile($data['microchip_id'])){
			return array("success"=>0,"message"=>"Microchip ID ".$data['microchip_id']." already used by another pet profile.");
		}
	}
	
	if (isset($data['smart_tag_id']) && !empty(trim($data['smart_tag_id']))) {
		if(!checkSmartIDTagPetProfile($data['smart_tag_id'])){
			return array("success"=>0,"message"=>"SmartTag ID ".$data['smart_tag_id']." already used by another pet profile.");
		}
	}
	...
	if (!empty($microchipId) && empty($smartTagId)) {
	    ...
	    if ($query->have_posts()) {
	    	...
	    	return array("success"=>0,"message"=>"Microchip id ".$microchipId." is not registered into our database so please register this id.");
	    }
	    ...
	    if ($query->have_posts()) {
	    	...
	    	return array("success"=>0,"message"=>"SmartTag id ".$smartTagId." is not registered into our database so please register this id.");
	    }
	}elseif (!empty($microchipId) && !empty($smartTagId)) {
		return array("success"=>0,"message"=>"Either update_pet_smart_tag_id or update_pet_microchip_id field is required.");
	}

	$createdBy = get_user_meta( $userId, 'created_by', true);

	if ($accountId != $createdBy) {
		return array("success"=>0,"message"=>"This pet profile is not created by you, so you will not be able to update this pet profile information.");
	}
    ...
    return array("success"=>1,"message"=>"Pet profile info updated successfully.");
}
...
function addPetProfileExistingUser($request){
    ...
    update_post_meta($postId , 'source', 'api');
...
function checkEmailExistBabelBark($request){
	...
	if ($accountId != $createdBy) {
		return array("success"=>0,"message"=>"This email is already exist in our database but it's not created by you.");
	}else{
		return array("success"=>1,"message"=>"This email is exist in our database.");
	}
}

function getCountryPhoneCode($request){
	return $countryPhoneCode = json_decode(file_get_contents(__DIR__."/../lib/countryPhoneCode.json"));
}
...
add_action('rest_api_init', function () {
    ...
    register_rest_route( 'babelbark/v1', 'owner-lookup',array(
        'methods'  => 'POST',
        'callback' => 'checkEmailExistBabelBark',
    ));

    register_rest_route( 'babelbark/v1', 'list-phone-country-code',array(
        'methods'  => 'GET',
        'callback' => 'getCountryPhoneCode',
    ));

    register_rest_route( 'smart-tag/v1', 'webhook',array(
        'callback' => 'checkWebhook',
    ));
});

function checkWebhook(){
	print_r($_REQUEST);
	wp_mail("rohit@vkaps.com", "webhook quickbook", print_r($_REQUEST, true));
}