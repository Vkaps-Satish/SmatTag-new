<?php 
/*
* Template Name:  importUserAfterFirstLogin
*/
...
set_time_limit(100);
 $log_filename = $_SERVER['DOCUMENT_ROOT']."/Querylog";
  if (!file_exists($log_filename)){
                    ...
                    //if($FistTimeLogIn  == 's' || $FistTimeLogIn  == 'sp'){
                        ...
                        $curl = curl_init();
                            ...
                            curl_setopt_array($curl, [
                                CURLOPT_URL => "http://3.80.120.98/basicRowScript.php?email=".$user_email,
   ...
   /*function to create professional users*/

 function PetsProfessionalSignup($finalData){
        ...
        $user_id      =  wp_create_user( $user_name , $random_password, $org_email );
         ...
         $userId = $user->ID;
            ...
            foreach ($professionData as $key => $value) {
             ...
             $log  = json_encode(array('success'=>1,'title'=>'Pet Professional','action' =>'updated','user_id' => $userId, 'message'=>'Account successfully updated.'));
...
/* function to create OwnerProfileUnderPetPro */
...
function createOwnerProfileUnderPetPro($finalData,$userid){
    ...
    $user_id      =  wp_create_user( $user_name , $random_password, $org_email );
        ...
        // update_user_meta( $user_id, 'sl_email_confirmation', 'false');
        // salient_register_emails_filter_replace($_POST['org_email']);
          $petID = importCreatePetProfile($finalData,$user_id);



           $log  = json_encode(array('success'=>1,'title'=>'Pet Owner','action' =>'created', 'user_id' => $user_id,'pet_id' => $petID,'message'=>'Account successfully created.'));
     ...
     $userId = $user->ID;
        ...
        $petID = importCreatePetProfile($finalData,$userId);
        // update_user_meta( $userId, 'sl_email_confirmation', 'false');
        $log_filename = $_SERVER['DOCUMENT_ROOT']."/Querylog";

          $log  = json_encode(array('success'=>1,'title'=>'Pet Owner','action' =>'updated','user_id' => $userId,'pet_id' => $petID, 'message'=>'Account successfully updated2.'));


            $log_file_data = $log_filename.'/log_' . date('d-M-Y') . '.log';
                    file_put_contents($log_file_data, $log . "\n", FILE_APPEND);

                     return json_encode(array('success'=>1,'title'=>'Pet Owner','action' =>'updated','user_id' => $userId,'pet_id' => $petID, 'message'=>'Account successfully updated2.'));
}
        ...
        $user_id      =  wp_create_user( $user_name , $random_password, $org_email );
         ...
         $user_id = $user->ID;
            ...
            // update_user_meta( $userId, 'sl_email_confirmation', 'false');
               ...
               $log  = json_encode(array('success'=>1,'title'=>'Pet Owner','action' =>'updated','user_id' => $user_id, 'pet_id' => $petID, 'message'=>'Account successfully updated3.'));
            ...
            return json_encode(array('success'=>1,'title'=>'Pet Owner','action' =>'updated','user_id' => $user_id, 'pet_id' => $petID, 'message'=>'Account successfully updated3.'));
