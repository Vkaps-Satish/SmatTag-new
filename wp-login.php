	...
	case 'rp':
		...
		if ( ( ! $errors->has_errors() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
			...
			setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
				...
				$login_url = get_site_url().'/login-to-smarttag/?login=1';

			/*custom URL change start--added by satish*/

			login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href='.$login_url.'>' . __( 'Log in' ) . '</a></p>' );
