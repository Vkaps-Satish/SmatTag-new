...
// print_r($finalDatas);die;
        $count = sizeof($finalDatas);
        $i = 0;
        while ($i < $count) {
            $finalData = array_map('trim',$finalDatas[$i]);
             // echo $pet_pro_email =  $finalData['Pet Shelter/ Vet Primary Business Email'];die;
            $user      = $this->PetProfessionalSignup($finalData);
            ...
            if($userId){
                $petID = $this->createOwnerProfileUnderPetPro($finalData,$userId);
                // $petID = $this->importCreatePetProfile($finalData,$userId);
                echo $userId."-".$petID;
            }
            else{
                echo  $petID = $this->createOwnerProfile($finalData);
            }
            $i++;
        }
        echo "success";
 ...
 $jsonString = file_get_contents('record_files/csvdetail.json');
        ...
        new csvDataUpload("record_files/new/updated-csv-".$number.".csv");
        $newData = array(
            'status'  => true,
            'csv number'   => $number,
        );
        echo $newJsonString = json_encode($newData);
        file_put_contents('record_files/csvdetail.json', $newJsonString);
